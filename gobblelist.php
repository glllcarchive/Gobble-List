...
function BuildList($list = array())
{
   ...
   if(! empty($items['table']))
	...
	if($count[0]['total'] > 0)
	{
      ...
      $pieces = array('<!--URL-->'  => $nextUrl,
                      '<!--TEXT-->' => $nextTxt);

      $nextBtn = Fill($pieces, $nextBtn);

      $pieces = array('<!--URL-->'  => $nextUrl,
                      '<!--TEXT-->' => $prevTxt);

      $prevBtn = Fill($pieces, $prevBtn);
      ...
      $paginationOutput = implode('', $paginationOutput);
		...
		{
		   $headers[] = Fill(array('<!--CONTENT-->' => $title), $widgetTemplate['header_row']);
		}
		...
		foreach($rows as $key => $row)
		{
		   ...
		   foreach($items['fields'] as $column => $title)
		   {
		      ...
		      $thisRow[] = Fill(array('<!--CONTENT-->' => $content), $widgetTemplate['list_data']);
		   }
		   ...
		   $pieces = array('<!--CLASS-->'   => $theClass,
		                   '<!--CONTENT-->' => implode('', $thisRow));

		   $dataRows[] = Fill($pieces, $widgetTemplate['list_row']);
	...
	else
	{
		$pieces = array('<!--HEADERS-->'           => '',
		                '<!--PAGINATION-->'        => $paginationOutput,
		                '<!--DATA-->'              => Fill(array('<!--CONTENT-->' => $items['noDataMsg']), $widgetTemplate['list_data_row']),
		                '<!--CLASS-->'             => $items['class'],
		                '<!--ID-->'                => $items['id'],
		                '<!--TITLE-->'             => $items['title'],
		                '<!--BOTTOM_PAGINATION-->' => $items['bottomNav']
	...
	$theList = Fill($pieces, $widgetTemplate['list_wrapper']);
